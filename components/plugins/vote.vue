<template>
    <div class="flex space-x-1 ml-auto pl-2 text-2xl items-center xs:text-xl mb-auto"
        ...
        <transition name="slide" mode="out-in">
            ...
            <div v-if="odds === 0" class="bg-gray-200 px-3 mx-0.5 relative text-base rounded dark:text-gray-300 dark:bg-gray-600"
                 :key="item.attributes.votes">
                <p class="font-medium">{{ odds }}</p>
            </div>
...
export default {
    ...
    methods: {
        ...
        toggleVote(id, type) {
            ...
            this.$axios.post('/articles/vote', {id: id, type: type})
                .then(response => {
                ...
                err.response.status === 401 ? this.$toast.error(this.$t('error.not_registered')) : null;
            })
        },
    }
}
