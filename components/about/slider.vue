    ...
    <div class="relative flex slider dark:slider overflow-hidden">
        <ul class="flex mb-2 left-0 w-full py-6 animation xs:hidden md:hidden sm:hidden">
            <li v-for="hub in hubs"
                class="flex flex-shrink-0 border dark:border-gray-700 mx-4 p-2 w-max bg-white rounded dark:bg-primary dark:text-gray-300">
                <img class="w-16 h-16 rounded p-1" :src="'/images/hubs/' + hub.logo + '.png'" alt="Hub logo">
            </li>
        </ul>
        <ul class="flex mb-2 left-0 w-full py-6 animation xs:hidden md:hidden sm:hidden">
            <li v-for="hub in hubs"
...
export default {
    ...
    methods: {
        async getHubs() {
            await this.$axios.get('/hubs/filter/all').then(({data}) => {
                for (let i = 0; i < 9; i++) {
...
.dark .dark\:slider:after {
    content: "";
    background: linear-gradient(to right, #0d1117 0, rgb(255 255 255 / 0%) 30%, rgb(23 28 32 / 0%) 70%, #0d1117 100%);
    position: absolute;
    left: 0;
    top: 0;
    right: 0;
