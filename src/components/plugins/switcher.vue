    ...
    <div class="mt-1 relative">
        <button type="button" aria-haspopup="listbox" aria-expanded="true"
            ...
            <span class="flex items-center">
                <icon class="iconify transition-none dark:text-gray-300"
                ...
                <ul class="max-h-56 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                    <li id="listbox-item-0" role="option"
                        ...
                        <div class="flex items-center cursor-pointer px-3 hover:bg-gray-300 group"
                            ...
                            <icon class="iconify transition-none dark:text-gray-300 dark:group-hover:text-black"
                                  icon="theme.icon" data-inline="false"></icon>
...
export default {
    ...
    created() {
        this.selected = localStorage.theme === 'dark' ? {'name': 'Dark', 'icon': 'bytesize:moon'} : {'name': 'Light', 'icon': 'carbon:sun'}
    },
    methods: {
        ChangeTheme(theme) {
